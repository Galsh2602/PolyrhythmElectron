...
export class Circle {
    constructor(radius, colorName = null, startX = 50, startY = 50, note = null) {
        ...
        this.startY = startY;
        this.note = note;
        this.lastPlayed = 0; // Timestamp of the last time the sound was played
        this.debounceTime = 500; // Debounce time in milliseconds
    ...
    // Method to play sound with debouncing
    playSound() {
        const now = Date.now();
        if (now - this.lastPlayed >= this.debounceTime) {
            ...
            this.glow(); // Trigger the glow effect when sound plays
        }
    }
}
